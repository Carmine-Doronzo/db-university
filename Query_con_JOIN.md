...
- SELECT `exam_student`.*,`exams`.`id`,`degrees`.`name` FROM `exam_student` JOIN `exams` ON `exam_student`.`exam_id` = `exams`.`id` JOIN `courses` ON `exams`.`course_id` = `courses`.`id` JOIN `degrees` ON `courses`.`degree_id` = `degrees`.`id` ORDER BY `exam_student`.`student_id`,`exam_student`.`exam_id` ASC; 